...
void trainN(int my_rank, int nprocs){
	...
	for( int i = 0; i < NUMOUT; i++)
    	...
    	for( int epoch = 0 ; epoch < 1000000; epoch++ ) 
        {
            ...
            MPI_Allreduce(WeightIH, aWeightIH, NUMHID*NUMIN, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
            MPI_Allreduce(WeightHO, aWeightHO, NUMHID*NUMIN, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
            MPI_Allreduce(&Error, &aError,  1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);

            Error = aError / nprocs;
            
            for( int j = 0 ; j < NUMHID ; j++ )     // Promig / nprocs
                for( int i = 0 ; i < NUMIN ; i++ )
                    WeightIH[j][i] = aWeightIH[j][i] / nprocs ;

            for( int k = 0 ; k < NUMOUT ; k ++ )    // Promig / nprocs
                for( int j = 0 ; j < NUMHID ; j++ )
                    WeightHO[k][j] = aWeightHO[k][j] / nprocs ;        
